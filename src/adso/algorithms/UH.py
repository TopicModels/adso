...
try:
    ...
    print(e)
    print(
        "hdbscan not found, UMAP_HDBSCAN algorithm not available.\nInstall it with conda (hdbscan package) should resolve the issue"
    )


class UMAP_HDBSCAN(TMAlgorithm):
    if "hdbscan" in modules:
        ...
        def fit_transform(
            ...
            clusterer = hdbscan.HDBSCAN(prediction_data=True, **self.h_args).fit(
                embedding
            )
